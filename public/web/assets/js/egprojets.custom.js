...
;(function ($, window, document, undefined) {
	...
	Egprojets = {
		...
		configGmaps: function(lat,longi){
			...
			if( $('#maps')[0] )
			{
				var latlng = new google.maps.LatLng("-8.119490","-79.034270");
			...
			$(document).on('submit','#form-contact form',function(){
				...
				if(!hasError) {
                    ...
                    $.post($(this).attr('action'), formInput, function (data) {
                        $('.overlay-f').remove();
                        if (data == 'email_error') {
                            $('#form-contact').append('<div class="mail-message"><p class="mail-head">Error!</p><p>Porfavor corrija su correo electrónico.</p></div>');
                        } else if (data == '1') {
                            $('.form-reservation').fadeOut("slow", function () {
                                $(this).before('<div class="mail-message"><p class="mail-head">Gracias!</p><p>Su correo electrónico ha sido entregado\n.</p></div>');
                            });
                        }
                    }, 'json').fail(function (error,a) {
                        $('.overlay-f').remove();
                        $('#form-contact').append('<div class="mail-message"><p class="mail-head">Error!</p><p>'+error.responseText+'</p></div>');
                    });
				}
				return false;
			});
